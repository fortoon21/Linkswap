...
import { IFlashLoanSimpleReceiver } from "../intf/IFlashLoanSimpleReceiver.sol";

import { IChainlink } from "../intf/IChainlink.sol";
import { IPool } from "../intf/IPool.sol";

import { IPoolAddressesProvider } from "../intf/IPoolAddressesProvider.sol";
 ...
 */
contract RouteProxyPolygon is IFlashLoanSimpleReceiver, Withdrawable, ReentrancyGuard {
    ...
    constructor(
        ...
        _LENDING_POOL_ = IPoolAddressesProvider(_ADDRESS_PROVIDER_).getPool();
    ...
    function _checkAndWrapETH(address token) internal view returns (address) {
        return token == _ETH_ADDRESS_ ? _WETH_ADDRESS_ : token;
    }
    ...
    function shieldSwap(
        ...
        if (flashDes.length > 0) {
            ...
            for (uint256 j; j < fd.uniqueNum; j++) {
                (, fd.tmp, , , ) = IChainlink(oracleRegistry[fd.uniqueAssets[j]]).latestRoundData();
                fd.assetValues[j] = fd.tmp.toUint256();
            }
            {
                (, fd.tmp, , , ) = IChainlink(oracleRegistry[_WETH_ADDRESS_]).latestRoundData();
                fd.nativePrice = fd.tmp.toUint256();
            }

            // realOutput = executeFlashloanV3(flashDes, fd, realOutput, toToken);
            output = executeFlashloanV3(flashDes, fd, output, toToken);
        }
        // emit OrderHistory(fromToken, toToken, msg.sender, amountIn, realOutput, output - realOutput);
    ...
    function executeFlashloanV3(
        ...
        for (uint256 i; i < flashDes.length; i++) {
            ...
            uint256 cmpOutput = flashDes[i].amountIn.mul(10000 + IPool(_LENDING_POOL_).FLASHLOAN_PREMIUM_TOTAL()).div(
                10000
            );
            ...
            if (
                ...
                IPool(_LENDING_POOL_).flashLoanSimple(
                    address(this),
                    flashDes[i].asset,
                    flashDes[i].amountIn,
    ...
    function _executeSwap(
        ...
        address to = toToken;
        console.log("RouteProxy._executeSwap:");
        console.log("RouteProxy._executeSwap: fromToken: %s", fromToken);
        console.log("RouteProxy._executeSwap: toToken: %s", toToken);
        console.log("RouteProxy._executeSwap: pool: %s", pool);
        console.log("RouteProxy._executeSwap: adapter: %s", adapter);
        console.log("RouteProxy._executeSwap: poolEdition: %d", poolEdition);
        ...
        console.log("RouteProxy._executeSwap: adapter.swapExactIn: start (adapter=%s)", adapter);
        output = IRouterAdapter(adapter).swapExactIn(from, amountIn, to, pool, address(this));
        console.log("RouteProxy._executeSwap: adapter.swapExactIn: done  (adapter=%s)", adapter);
